...
export function buildTripPayload(form, apiResults) {
  return {
    ...
    budget: form.budget === 'custom' ? `custom:${form.customBudget}` : form.budget,
  ...
  return {
    ...
    budget: {
      ...
      breakdown: [
        ...
        { label: 'Hotels', amount: (tripPayload.hotels?.[0]?.pricePerNight ?? 120) * days.length },
    ...
    bestPlaces: [
      ...
      },
      {
        ...
        type: interests.has('beach') || interests.has('nature') ? 'nature' : 'relaxation',
...
function buildActivities(interests, dayIndex) {
  const pool = [
    ...
    interests.has('family-friendly') ? 'Family-friendly interactive exhibit' : null,
...
export function createAiService({ provider }) {
  return {
    ...
    async generatePlan(tripPayload) {
      if (provider === 'mock' || !config.OPENROUTER_API_KEY || config.OPENROUTER_API_KEY.includes('your_')) {
      ...
      const response = await fetch('https://openrouter.ai/api/v1/chat/completions', {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: 'system',
              content: 'You are a travel planner. Respond with valid JSON only.',
            },
